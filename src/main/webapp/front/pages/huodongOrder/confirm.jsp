		...
		<script>
			var vue = new Vue({
				...
				computed: {
					totalMoney: function() {
						...
						for (var item of this.dataList) {
							total += item.huodongNewMoney * item.buyNumber
						}
						return total;
					}
				},
				methods: {
					...
					,payClick() {
						let data={
                            huodongs:localStorage.getItem('huodongs'),
                            yonghuId: localStorage.getItem('userid'),
						}
                        // 获取商品详情信息
                        layui.http.request(`huodongOrder/order`, 'POST', data, (res) => {
                            // 订单编号
			...
			layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload'], function() {
                ...
                // 获取活动购买的清单列表
				var huodongs = localStorage.getItem('huodongs');
				// 转换成json类型，localstorage保存的是string数据
				vue.dataList = JSON.parse(huodongs);
